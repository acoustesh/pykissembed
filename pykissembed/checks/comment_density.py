...
def _code_body_lines(node: ast.FunctionDef | ast.AsyncFunctionDef) -> int:
    ...
    # Subtracts exactly 1 line for the signature, even though a signature
    # with a multi-line parameter list spans more than one physical line.
    # That's an accepted approximation: this count only feeds the
    # small-function *exemption* threshold below, not a hard pass/fail
    # metric, so slightly overcounting a wrapped signature as "body" is
    # harmless.
    code_lines = total - 1
    body = node.body
    if (
...
def _all_functions_short(file_path: Path, max_lines: int = SMALL_FUNCTION_THRESHOLD) -> bool:
    ...
    if not functions:
        return False
    # Files made up entirely of small functions (thin wrappers, dataclass-
    # style boilerplate, __init__.py re-exports) can't sustain a 5% comment
    # floor without padding real code with filler comments just to satisfy
    # the metric — so they're exempted from the density check entirely
    # rather than penalized for being simple.
        ...
        *,
        ...
        with locked_envelope(baseline_file, kind="density") as envelope:
            ...
            if update_baselines:
                ...
                # to admit the current value (not reset to it) — files already
                # inside the default range keep using it, so a future ratchet
                # of the global thresholds still tightens them automatically.
