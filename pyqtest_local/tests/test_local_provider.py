...
from pyqtest_local.provider import DEFAULT_MODEL_ID, LocalProvider
from pyqtest_local.runner import cache_key, populate, truncate_to_tokens
...
class TestProviderAttributes:
    ...
    def test_satisfies_provider_protocol() -> None:
        """The provider structurally satisfies the pyqtest Provider Protocol."""
...
class TestEntryPoints:
    ...
    def test_entry_point_registered() -> None:
        """The local entry point is registered for the pyqtest.providers group.

        Both the core stub (``pyqtest.providers.local``) and the real
        implementation (``pyqtest_local.provider``) declare an entry
        ...
        eps = {ep.name: ep.value for ep in metadata.entry_points(group="pyqtest.providers")}
        assert "local" in eps
        assert eps["local"] in {
            "pyqtest_local.provider:LocalProvider",
            "pyqtest.providers.local:LocalProvider",
        }
