...
The core ``pyqtest`` CLI delegates ``populate-embeddings`` here via:

    from pyqtest_local.runner import populate

This module is the only piece of ``pyqtest-local`` that walks the
...
class ProviderLike(Protocol):
    """Minimal protocol — duck-types ``pyqtest.providers.Provider``."""
...
def _get_tiktoken() -> Any:
    ...
    if _tiktoken is None:
        try:
            ...
            msg = "pyqtest-local requires tiktoken for token-aware truncation"
...
def cache_key(provider: ProviderLike, text: str) -> str:
    """Compose the (provider, content) cache key — matches pyqtest's format.
