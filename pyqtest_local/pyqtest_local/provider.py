...
The core ``pyqtest`` package ships a stub under ``pyqtest.providers.local``
that raises a clear error when ``pyqtest-local`` isn't installed. This
subpackage provides the real implementation and registers it via the
``pyqtest.providers`` entry-point group, so the stub is overridden
...
class LocalProvider:
    ...
    def embed(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        ``pyqtest providers list`` instant).
