...
Every baseline file written or read by pyqtest is wrapped in a versioned
envelope::
...
Validation is performed against ``pyqtest/schemas/baselines.v1.json`` at
...
# Lazy-loaded validator — schemas are bundled in pyqtest/schemas/
...
def _load_validator() -> Draft7Validator:
    ...
    if _VALIDATOR is not None:
        return _VALIDATOR
    schema_text = (
        resources.files("pyqtest.schemas").joinpath("baselines.v1.json").read_text("utf-8")
    )
