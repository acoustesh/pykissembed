"""Load ``[tool.pyqtest]`` configuration from ``pyproject.toml``.

pyqtest follows a layered configuration model (per advisor recommendation):

1. ``[tool.pyqtest]`` block in ``pyproject.toml`` (explicit, wins)
...
class PyqtestConfig:
    """Resolved pyqtest configuration.
    ...
    cache_dir
        ...
        gitignored). Defaults to ``tests/.pyqtest_cache``.
    root
    ...
    cache_dir: str = "tests/.pyqtest_cache"
    ...
    if isinstance(value, list):
        ...
        for item in value:
            if not isinstance(item, str):
                msg = f"[tool.pyqtest] {key!r} must be list[str], got list containing {type(item).__name__}"
                raise TypeError(msg)
            result.append(item)
        return result
    return []
...
def load_config(start: Path | None = None) -> PyqtestConfig:
    """Resolve pyqtest configuration.

    Walks up from ``start`` (default: cwd) looking for a ``pyproject.toml``
    that contains a ``[tool.pyqtest]`` block. Falls back to auto-detection
    ...
    cache_dir = str(section.get("cache_dir", "tests/.pyqtest_cache"))
...
def _auto_detect(root: Path) -> PyqtestConfig:
    ...
    return PyqtestConfig(
        paths=paths,
        mode=os.environ.get("PYQTEST_MODE", "ratchet"),
...
def get_config() -> PyqtestConfig:
    """Return the cached pyqtest configuration.

    Uses :func:`load_config` on first call and caches the result in
    ``PYQTEST_CONFIG_CACHE`` for subsequent calls within the same process.
    """
    cache: dict[str, PyqtestConfig] = globals().setdefault("PYQTEST_CONFIG_CACHE", {})  # type: ignore[var-annotated]
    key = str(Path.cwd())
    if key not in cache:
...
def reset_config_cache() -> None:
    ...
    cache = globals().get("PYQTEST_CONFIG_CACHE")
