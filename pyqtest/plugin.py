...
def pytest_addoption(parser: pytest.Parser) -> None:
    """Add pyqtest's custom CLI options to pytest."""
    parser.addoption(
        "--update-baselines",
        action="store_true",
        default=False,
...
def pyqtest_paths() -> list[Path]:
    """Resolved list of source directories from ``[tool.pyqtest]``.
    ...
    """
    from pyqtest.paths import resolve_paths
...
def pytest_configure(config: pytest.Config) -> None:
    """Register pyqtest markers and ensure source dirs are importable."""
